 ...
 */
int esp_console_init(const esp_console_config_t *config);
 ...
 */
int esp_console_run(const char *cmdline, int *cmd_ret);
...
struct esp_console_repl_s {
     ...
     *      - ESP_FAIL on errors
     */
    int (*del)(esp_console_repl_t *repl);
};
 ...
 *      - ESP_FAIL Parameter error
 */
int esp_console_new_repl_usb_serial_jtag(const esp_console_dev_usb_serial_jtag_config_t *dev_config, const esp_console_repl_config_t *repl_config, esp_console_repl_t **ret_repl);
#endif // CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG || (defined __DOXYGEN__ && SOC_USB_SERIAL_JTAG_SUPPORTED)
 ...
 */
int esp_console_start_repl(esp_console_repl_t *repl);
